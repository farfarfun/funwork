...
> 注意：PyPI 上目前**没有**发布本仓库对应的 `funwork` 包。PyPI 上确实存在一个叫 `funwork`（0.0.1）的包，但经核对其 wheel 内容只有一个空的 `funapi/__init__.py`，是历史上批量占位发布的空包，**和这个仓库的代码毫无关系**，请不要用它来安装本仓库的功能。
...
```python
from funwork.youzan.dubbo import dubbo
...
```python
from funwork.youzan.pdf2html import pd2html
import pandas as pd
...
- `funwork/ItemUtils.py` 里的 `fill_item_info` 读取 `r.data_mock`，但 `urllib3` 的响应对象并没有 `data_mock` 这个属性（正常应为 `r.data`），这个函数实际运行会报错，属于遗留 bug，本次只做文档说明，不在本次改动范围内修复。
- `funwork/youzan/items.py` 里的接口地址、鉴权 Token、Cookie 都是作者本人的加密凭据，外部使用者无法直接调用。
